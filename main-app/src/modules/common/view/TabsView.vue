    ...
    <el-tabs class="main-tab" v-model="activeTab" type="border-card" @tab-remove="removeTab">
      <el-tab-pane
        ...
        <span slot="label" class="label">{{item.title}} <i @click="refresh(item.id)" v-if="selectedTab === item.id" class="el-icon-refresh"></i></span>
          <component :is="$tab.getComponent(item)" class="content-view" v-if="!isChildApp(item)"></component>
          <div class="content-view" v-html="getSubAppHtml(item)" v-else></div>
      </el-tab-pane>
    </el-tabs>
...
export default @Component class TabsView extends Vue {
  ...
  @State((state) => state.subAppHtml) subAppHtml
  ...
  isChildApp (tab) {
    return (tab.components[tab.components.length - 1] || '').startsWith(process.env.VUE_APP_PRE_FIX)
  }
  getSubAppHtml (tab) {
    if (this.selectedTab === tab.id) {
      return this.subAppHtml
    } else {
      return ''
    }
  }
